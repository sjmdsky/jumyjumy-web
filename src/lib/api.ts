...
export interface FetchQuestionOptions {
   ...
   *
   * 存在的理由与 [`userAgent`] 同源：后端按 IP 限流，而它自己收到的
   * `cf-connecting-ip` 是边缘在「Worker 打后端」那一跳上写的，也就是本
   * Worker 的地址。不转发的话，所有访客在后端眼里共用一个 IP、共用一个
   * 限流桶——一个人打满，全站一起被限。访客的真实 IP 只有这一跳知道。
   *
   * 发出去用 `x-real-ip` 这个名字不是随意的，实测过：边缘会把自家 Worker
   * 发来的 `x-real-ip` **提升成**下一跳的 `cf-connecting-ip`，同时把
   * `x-real-ip` 本身改写成那一跳的对端地址。所以后端读的是
   * `cf-connecting-ip` 而不是这个头——它到了那边已经不是访客的地址了。
   * 反过来，后端那侧若把 `x-real-ip` 配成取值来源，拿到的会是 172.70.x.x
   * 这样的边缘地址。这个坑踩过，改这个头名之前先读完这一段。
   *
   * 提升只认自家 Worker：外部直接向后端域名发一个伪造的 `x-real-ip` 会被
   * 边缘丢弃，`cf-connecting-ip` 仍是它实际看到的连接地址。因此这条链路上
   * 的访客 IP 既是真的，也伪造不了。
   */
  readonly clientIp?: string | null
  /**
   * 与后端共享的密钥，放在 `x-gateway-token` 头里。
   *
   * 后端的 `/q/` 只接受带对密钥的请求，其余一律 401。后端域名不出现在任何
   * 公开产物里，唯一的合法调用方就是本 Worker；但域名会经证书透明日志暴露，
   * 而后端每建一个新问题都要付一次模型调用并写一行永久记录。
   *
   * 它与 [`clientIp`] 要么一起发要么都不发。这个绑定现在只是取舍上的简化，
   * 不再有安全含义——访客 IP 的可信度来自边缘的提升，不来自这把密钥；密钥
   * 唯一的职责是给 `/q/` 把门。后端未配置密钥时保持开放，所以两边先后上线
   * 的中间态不会白屏。
   */
