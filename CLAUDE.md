...
The apex `jumyjumy.com` does not resolve — only `www` is bound, so there is no apex→www redirect yet. Not yet built: `robots.txt` (when it lands it must `Disallow: /api/` and `Disallow: /q/pending/` — the skeleton route is reachable directly and is only kept out of the index by its `noindex` tag today), a sitemap (needs a backend endpoint listing indexable questions), and any analytics or view counting. `Question.fingerprint` now comes from the backend's dedup key, but nothing in the frontend reads it — dedup happens server-side.
