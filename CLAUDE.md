...
- **`excerpt.ts`** — `toPlainText` / `buildExcerpt` distil the answer markdown into the one-sentence `<meta name="description">` that social cards and search snippets both read. Inline marks (bold, inline code, links) keep their text and lose their syntax; purely structural content (fenced code, tables) is dropped whole, because a sysctl command or half a pipe grid inside 150 characters makes the card look broken. Table detection anchors on the **delimiter row**, not on pipes, matching `markdown.ts`'s `splitTableRow` (which accepts GFM's optional outer pipes) — anchoring on pipes alone would both miss tables the renderer accepts and eat prose like `ls | grep`. Sentence-end detection runs against the **full text**, never the clipped slice: a `$` anchored to the artificial cut point accepts the period in `4.9` whenever it lands on character 150, which is the exact case the rule exists to reject. The clip also strips a trailing lone surrogate, or an emoji straddling the limit puts half a code point into the HTML head. HTML stripping is tag-shaped (`/<\/?[a-zA-Z][^>]*>/`) rather than `/<[^>]*>/`, which would silently delete the middle of "kernel < 4.9 and memory > 2 GB". Returns `''` on empty input so the caller can fall back — callers must fall back to the **site copy, never the title**.
...
A forwarded link renders a card with three fields — title, description, thumbnail — and each is fed by a different mechanism. **WeChat reads `<title>` and `<meta name="description">`, and picks its thumbnail from a real `<img>` in the DOM. It does not read `og:image`.** The `og:*` and `twitter:*` block in `BaseLayout.astro` serves Twitter / Slack / Feishu; it cannot serve WeChat.

That asymmetry is why `q/[slugId].astro` carries a brand mark in its footer. Before it, nothing on the site rendered an `<img>` at all (`Logo.astro` is inline SVG), so a forwarded answer showed a blank grey thumbnail. Three constraints follow and must not be relaxed: the image has to be **genuinely rendered** (`display:none` is skipped), it carries **no `loading="lazy"`**, and the page must contain **no second image tag — and no comment spelling that tag out**, since a string-scanning picker hits the decoy first and finds no `src`.

`public/share-card.png` is a 600x600 tile: `icon.svg`'s rounded dark plate under `logo-dark.svg`'s full wordmark. Square because WeChat crops to square in every surface, so a 4.18:1 wordmark would lose both ends; PNG because WeChat's thumbnail rejects SVG; and it carries its own background because one `<img>` is one bitmap and cannot follow `prefers-color-scheme` — a transparent wordmark would vanish under one theme or the other. That `logo.svg` and `logo-dark.svg` exist as two files is the proof.

The description comes from `buildExcerpt(question.answerMarkdown)`, and falls back to the site copy — **never to the title**, which is what put the same sentence in both card rows before this existed. `q/pending/[query].astro` now omits the prop for the same reason.

The file has no content hash, so `_headers` gives it a one-week cache rather than the `immutable` year that `/_astro/*` gets. Replacing the image leaves stale copies in crawlers and browsers for up to a week — accepted deliberately.
...
145 tests pass; coverage of `src/lib/` is 98.33% statements / 89.26% branches (`types.ts` reports 0% because it is type declarations only).

The apex `jumyjumy.com` does not resolve — only `www` is bound, so there is no apex→www redirect yet. Built: `favicon.ico` multi-resolution icon in `public/favicon.ico`, `robots.txt` in `public/robots.txt` (includes Cloudflare Content Signals policy reservation comments, `Content-signal: search=yes, ai-input=yes, ai-train=no`, `Disallow: /api/`, `Disallow: /q/pending/`, and `Sitemap: https://www.jumyjumy.com/sitemap.xml`), visitor IP forwarding (`cf-connecting-ip` via `x-real-ip`) with shared `x-gateway-token` authorization, `/sitemap.xml` dynamic endpoint with Cloudflare Cache API edge caching, pagination validation, and 503 fail-safe semantics, and social share cards (answer-derived `<meta name="description">`, `og:*`/`twitter:*` block, and a 600x600 brand tile served as both `og:image` and the in-body `<img>` WeChat picks from). Verified in production for the share work: `/share-card.png` returns 200 `image/png` with the `_headers` cache policy applied, a live answer page carries exactly one `<img>` pointing at it, and the header ask-box renders empty. Not yet built: any analytics or view counting. `Question.fingerprint` now comes from the backend's dedup key, but nothing in the frontend reads it — dedup happens server-side.
