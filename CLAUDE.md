...
`API_BASE_URL` is declared in `astro.config.mjs` via `astro:env` (`envField.string`) and read by pages through `import { API_BASE_URL } from 'astro:env/server'`.
...
How the value is resolved, verified by test:

| Mode | Source |
| :--- | :--- |
| `astro dev` / `astro build` | `.env` |
| deployed Worker | a secret set with `wrangler secret put API_BASE_URL`; the adapter copies every `env.schema` key into `process.env` per request |
| `wrangler dev` | `.dev.vars` — **not** `.env` |
| nothing set | schema `default`, `http://localhost:3000` |

The value deliberately does **not** live in `wrangler.jsonc` `vars`. This repository is public, and `vars` is plaintext: committing the backend origin there publishes an unauthenticated endpoint that spends model quota on every call. A secret is the only source that keeps it out of the tree while still reaching `process.env` per request. With nothing configured, `/q/*` returns 502 across the board — that is the expected symptom of a missing config, not a bug.
...
85 tests pass; coverage of `src/lib/` is 97.6% statements / 90.8% branches (`types.ts` reports 0% because it is type declarations only).

The apex `jumyjumy.com` does not resolve — only `www` is bound, so there is no apex→www redirect yet. Built: `robots.txt` in `public/robots.txt` (includes Cloudflare Content Signals policy reservation comments, `Content-signal: search=yes, ai-train=no`, `Disallow: /api/`, and `Disallow: /q/pending/`). Not yet built: a sitemap (needs a backend endpoint listing indexable questions), and any analytics or view counting. `Question.fingerprint` now comes from the backend's dedup key, but nothing in the frontend reads it — dedup happens server-side.
