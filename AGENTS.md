...
`jumyjumy-web` 的第一定位是 **Real-time AI Search（实时 AI 搜索）**：用户搜索任何问题，后端实时检索全网并给出带信源引用的直接答案，每个答案沉淀为可被 Google 收录的永久页面。本仓库是它面向 SEO 优化的高性能极简英文前端（**ONLY English**，全站 UI、元数据与内容均为纯英文）。对外文案（标题、meta description、标语、状态提示）统一强调“实时、全网、有信源”，不要写成聊天机器人或问答社区。
全站仅包含两种核心页面类型：
1. **首页**：极简居中的实时搜索框，配备 `Real-time AI Search` 占位提示与日落流光边框动效。
